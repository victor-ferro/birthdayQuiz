...
import { getLeaderboard, pts } from '@/lib/leaderboard'
...
function PodiumSlot({
  ...
  const colBg = {
    1: 'rgba(0,64,224,0.12)',
    2: 'rgba(180,0,103,0.1)',
    3: 'rgba(201,169,0,0.1)',
  }[rank]
  ...
  return (
    <div
      className={`flex flex-col items-center ${rank === 1 ? 'flex-[1.2] -translate-y-5' : 'flex-1'}`}
    >
      {rank === 1 && (
        <Crown className="w-6 h-6 mb-1 animate-bounce" style={{ color: '#c9a900' }} />
      )}
      <AvatarCircle
        name={entry.name}
        className={`${avatarSize} mb-1 ${isMe ? 'ring-2 ring-offset-1 ring-primary' : ''}`}
      />
      <p className="text-xs font-bold text-center w-full truncate px-1 text-foreground">
        {entry.name}
      </p>
      <p className="text-xs font-semibold mb-1" style={{ color: ptsColor }}>
        {pts(entry)} pts
      </p>
      <div
        className={`w-full ${colHeight} rounded-t-lg flex items-end justify-center pb-1`}
...
export function LeaderboardScreen({ playerName, onPlayAgain }: Props) {
  ...
  function isMe(e: LeaderboardEntry) {
    return e.name.toLowerCase() === playerName.toLowerCase()
  }
    ...
    <div className="w-full max-w-sm mx-auto animate-fade-in pb-6">
      ...
      <div className="text-center mb-6 pt-2">
        <div className="flex justify-center items-end gap-4 mb-3">
          <div className="group w-16 h-16 rounded-full overflow-hidden border-2 border-yellow-300 shadow-md transform -rotate-6 hover:-translate-y-1 transition-transform duration-300">
            <img
        ...
        <h2 className="text-2xl font-extrabold tracking-tight" style={{ color: '#0040e0' }}>
          Leaderboard
        </h2>
        ...
        <div className="space-y-2 mb-6">
          {rest.map((entry, i) => (
            ...
            >
              ...
              <span className="text-sm font-bold" style={{ color: '#0040e0' }}>
                {pts(entry)} pts
              </span>
            </div>
          ))}
      ...
      <div className="flex justify-center">
        <button
          ...
          style={{
            backgroundColor: '#b40067',
            boxShadow: '0 10px 20px rgba(180,0,103,0.3)',
          }}
        >
